...
const errorResponse = (status) =>
  new globalThis.Response(JSON.stringify({ error: { message: "synthetic failure" } }), {
    status,
...
test("GET retries transient failures, disposes retry bodies, then succeeds", async () => {
  ...
  globalThis.fetch = async (_url, init) => {
    ...
    calls++;
    if (calls === 3) return new globalThis.Response(JSON.stringify({ items: [] }), { status: 200 });
    return new globalThis.Response(new globalThis.ReadableStream({ cancel: () => disposed++ }), {
...
test("401 disposes its body before reporting expired authentication", async () => {
  ...
  globalThis.fetch = async () => {
    calls++;
    return new globalThis.Response(new globalThis.ReadableStream({ cancel: () => disposed++ }), { status: 401 });
  };
  try {
