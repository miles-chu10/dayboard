...
const message = { id: "message", role: "assistant", content: "Stopped reply", tools: [], actions: [], error: null, blocked: null };

async function fixture(invoke) {
  const calls = [];
  globalThis.__historyClient = { invoke: (channel, input) => { calls.push({ channel, input }); return invoke(channel, input); } };
  const stubs = {
    react: "export const useState = init => [typeof init === 'function' ? init() : init, () => {}]; export const useRef = value => ({current: value}); export const useEffect = () => {};",
...
test("message changes reach the backend immediately and selection waits for acknowledgement", async () => {
  ...
  try {
    const { client, calls } = await fixture(channel => channel === "assistant:saveChat" ? pending : Promise.resolve(initial));
  ...
  } finally { delete globalThis.__historyClient; }
});

test("a failed save preserves the current draft and blocks switching until retry succeeds", async () => {
  let failing = true;
  try {
    const { client, calls } = await fixture(() => failing ? Promise.reject(new Error("synthetic disk failure")) : Promise.resolve(initial));
    client.setMessages([message]);
    assert.equal(await client.selectChat(null), false);
    assert.equal(calls.some(call => call.channel === "assistant:selectChat"), false);
    failing = false;
    await client.flush();
