...
- Version: `1.3.0-beta.2`; packaged target: Apple Silicon, macOS 14+.
...
- 10 renderer tests and 178 backend/native tests passed. The separate billing package passes 16 tests, including real local workerd/Miniflare + D1 execution, Stripe SDK serialization/signature verification, desktop activation and refund invalidation. All provider traffic is intercepted in those tests.
...
- A fresh dependency installation passed the baseline standalone checks. The beta.2 preview DMG and ZIP include the arm64 helper, GPL text and third-party notices. Stripe SDK/backend credentials are excluded from the Mac app.
...
- The earlier beta.1 packaged fictional demo reached Agenda in 1.7 seconds with warm OS caches; summed process working sets were 366 MiB. Beta.2 was not re-benchmarked. This is not a real-account benchmark or unique physical-memory measurement.
