...
// Brand marks from Simple Icons (CC0). Claude keeps its brand coral; OpenAI follows text color.
const CLAUDE_PATH =
...
/** Logo on a soft rounded tile, for empty states and headers. */
export function ProviderTile({
  ...
  return (
    <div
      className={cn(
        "shrink-0 flex items-center justify-center bg-control-subtle border border-separator",
        size === "large" ? "size-16 rounded-2xl" : "size-8 rounded-lg",
      )}
    >
      <ProviderMark provider={provider} className={size === "large" ? "size-9" : "size-4.5"} />
    </div>
  );
}
